...
nrows = fetch(dbSendQuery(con, 'SELECT COUNT(*) FROM notes20150306.rtfnotes'))

nrows = fetch(dbSendQuery(con, 'SELECT COUNT(*) FROM notes20150306.notetxt'))

nr = as.numeric(nrows)
print(nr)
 # for (i in 1390156:nr) {
     query = paste('SELECT nt.notetxt FROM notes20150306.rtfnotes rn, notes20150306.notetxt nt WHERE rn.de_id = nt.de_id AND documentid = ', documentid, ' LIMIT 100;')
     # documenttype
       # de_id
